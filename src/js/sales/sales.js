  ...
  function printTicket(event) {
    ...
    fetch(PATH, {
      ...
      .then((response) => {
        ...
        docAux = new jsPDF();
        docAux.setFont('Dosis', 'normal');
        docAux.addFont('dosis-regular.ttf', 'Dosis', 'normal');
        docAux.setFontSize(7);


        //  Calcula el alto requerido para el ticket
        for (let i = 0; i < totalCartridges; i++) {
          let text = ticketDetails['cartridges'][i]['name'].toUpperCase(),
            lines = docAux.splitTextToSize(text, 25);
          heightDoc += (3 * lines.length) + 1;
        }

        //  Calcula el alto requerido para el ticket
        for (let i = 0; i < totalPackages; i++) {
          let text = ticketDetails['packages'][i]['cartridges'];
          let name = text[0] + text[1] + text[2];
          let lines = docAux.splitTextToSize(name.toUpperCase(), 25);
          heightDoc += (3 * lines.length) + 1;
        }
        ...
        doc.addImage(imgData, 'JPEG', 17 , 4 , 24, 24);
        ...
        for (let i = 0; i < totalCartridges; i++) {
          let text = ticketDetails['cartridges'][i]['name'].toUpperCase(),
            lines = doc.splitTextToSize(text, 25);
          doc.text(3, tableTop, lines);
          tableTop += (3 * lines.length) + 1;
        }

        for (let i = 0; i < totalPackages; i++) {
          let text = ticketDetails['packages'][i]['cartridges'];
          let name = text[0].substr(0,3) + ' ' + text[1].substr(3,2) + ' ' + text[2].substr(0,2);
          let lines = docAux.splitTextToSize(name.toUpperCase(), 25);
          doc.text(3, tableTop, lines);
          tableTop += (3 * lines.length) + 1;
        }

        doc.text('ID: ' + ticketDetails['ticket_id'], 3, tableTop + 10, null);
        doc.setFontSize(6);
        doc.text('Whatsapp: 55 8107 4807', 29, tableTop + 14, null, null, 'center');
        doc.setFont('Dosis', 'bold');
        doc.setFontSize(8);
        doc.text('¡GRACIAS POR SONREIR! :D', 29, tableTop + 19, null, null, 'center');

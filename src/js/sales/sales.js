  ...
  function printTicket(event) {
    ...
    fetch(PATH, {
      ...
      .then((response) => {
        ...
        doc.addImage(imgData, 'JPEG', 8 , 4 , 42, 42);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('ORDEN ' + ticketDetails['ticket_order'], 29, 50, null, null, 'center');
        doc.setFont('courier', 'normal');
        doc.setFontSize(6);
        doc.text('Calle Hidalgo #78, San Lucas Tepetlaco,', 29, 55, null, null, 'center');
        doc.text('54055, Tlalnepantla, Méx\'', 29, 58, null, null, 'center');
        doc.setFont('courier', 'bold');
        doc.setFontSize(7);
        doc.text('NOMBRE            COST  CANT  TOTAL', 3, 65);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(7);
        doc.text('Whatsapp: 55 8107 4807', 29, 90, null, null, 'center');
        doc.setFont('times', 'italic');
        doc.setFontSize(7);
        doc.text('¡GRACIAS POR SONREIR!', 29, 100, null, null, 'center');

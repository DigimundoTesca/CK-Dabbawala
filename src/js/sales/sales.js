  ...
  function printTicket(event) {
    ...
    fetch(PATH, {
      ...
      .then((response) => {
        ...
        //Calculate the legth of the thicket
        var legth_ticket=50
        for(let productos of response.ticket_details.cartridges){
          legth_ticket=legth_ticket+4;
        }
        for(let pack of response.ticket_details.packages){
          legth_ticket=legth_ticket+9;
        }
        //Function to print a line with some fixed errors of jspdf arguments(var type jsPDF, CORDENATE X FROM BEGIN, CORDENATE Y FROM BEGIN,CORDENATE X TO END, CORDENATE Y TO END,LEGHT)
        //If the line its more bigger than the space betwen coords the legth will put in segments.
        var doc = new jsPDF('p','mm',[58,legth_ticket]);
        ...
        //Ticket start 
        doc.addFont('sans-serif','Comic Sans','normal');
        doc.setFont('Comic Sans');
        doc.addImage(imgData, 'JPEG',18 , 1, 20, 20)
        doc.text('ORDEN: ',14,28);
        doc.text(response.ticket_details.ticket_order.toString(), 35, 28);
        doc.setFontSize(6);
        doc.text('Calle Hidalgo #78, San Lucas Tepetlaco. 54055',7,35,0,0)
        doc.text('Tlalnepantla.Méx',21,38)
        doc.text('NOMBRE\t\t\tCOSTO\tCANT\tTOTAL',2,43);
        dottedLine(doc,2,44,55,44,48);
        //Products info
        var posy = 47;var total_sell=0;
        for(let products of response.ticket_details.cartridges){
          if((products.name.length) % 22==0){
            let line_break= products.name[22] + "\n"
            doc.text(line_break,2,posy);
          }
          else{
          doc.text(products.name,2,posy);}
          let unitary_price = (parseFloat(products.total)/parseFloat(products.quantity));
          doc.text('$',27,posy);doc.text(unitary_price.toString(),29,posy);
          doc.text(products.quantity.toString(),40,posy);
          doc.text('$',48,posy);doc.text(products.total.toString(),50,posy);
          posy=posy+3;
          total_sell=total_sell+parseFloat(products.total);
        }
        //Package info
        for(let pack of response.ticket_details.packages){
          dottedLine(doc,2.5,posy-2,55,posy-2.5,48);
          let unitary_price = (parseFloat(pack.total)/parseFloat(pack.quantity));
          doc.text('$',27,posy);doc.text(unitary_price.toString(),29,posy);
          doc.text(pack.quantity.toString(),40,posy);
          doc.text('$',48,posy);doc.text(pack.total.toString(),50,posy);
          for(let product_pack of pack.cartridges){
            doc.text(product_pack,2,posy);
            posy=posy+3;
          }
          total_sell=total_sell+parseFloat(pack.total);
        }
